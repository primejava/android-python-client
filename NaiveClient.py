...
import subprocess
import ctypes
from Wrapper import AVFormatContext, AVCodecContext, AVPacket, AVFrame, read_packet_func
...
class ScrcpyDecoder:
    ...
    #只有一个作用 在连接手机后接收固定长度流,打印连接设备的信息
    def _receive_info(self):
        ...
        self.frame_width, self.frame_height = struct.unpack(">HH", res)
        print("WxH: " + str(self.frame_width) + "x" + str(self.frame_height))


class NaiveScrcpyClient:
    def __init__(self, _config,_record):
        self.config = _config
        self.record=_record
        #lib_path如果不存在,则值为lib
        ...
        self.device_serial = _config.get('device_serial')
    ...
    #连接到手机并且执行了三条命令,推送jar包,建立转发通道,在手机上启动jar服务(就和在服务器上部署网站项目一样一样的)
    #1.adb push scrcpy-server.jar /data/local/tmp/
    #2. adb forward tcp:61550 localabstract:scrcpy
    #3.adb shell CLASSPATH=/data/local/tmp/scrcpy-server.jar app_process /  com.genymobile.scrcpy.Server 1280 2**30
    def _connect_and_forward_scrcpy(self):
        try:
            print("Upload JAR...")
            adb_push = subprocess.Popen(
                [self.adb,"-s",self.device_serial, 'push',
                 'scrcpy-server.jar',
                 '/data/local/tmp/'],
            ...
            subprocess.call(
                [self.adb,"-s",self.device_serial, 'forward',
    ...
    def _disable_forward(self):
        subprocess.call(
            [self.adb,"-s",self.device_serial, 'forward', '--remove',
             'tcp:%d' % self.port])
    ...
    def _playback(self,_count,_quicken=False):
        last_time = 0
        contents=self.record.file_to_content()
        for _ in range(_count):
            try:
               for idx,item in enumerate(contents):
                   ...
                   if idx>0:
                       #极速模式下 间隔固定为0.01秒
                       interval = (0.01 if _quicken else (_time - last_time))
                       time.sleep(interval)
                   last_time = _time
                   self.decoder.send_data(data)
    ...
    def _poweroff(self):
        try:
            content = self.record.bytes_content(method="toggleScreenPower", type=5, action=0)
            self.decoder.send_data(content)
        except Exception as e:
            print(e.args)
            print("==============")
            print(traceback.format_exc())

    def playback(self,count,_quicken):
        Thread(target=self._playback, args=(count,_quicken,)).start()
